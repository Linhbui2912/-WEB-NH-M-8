...
$viewerId = $_GET['viewer'] ?? (string) $_SESSION['user_id'];
...
try {
    ...
    if (!$profileUserId && $username === null) {
        $stmt = $pdo->prepare('
            SELECT tenDangNhap FROM NguoiDung
            WHERE maNguoiDung = :id AND trangThai = \'hoat_dong\'
            LIMIT 1
        ');
        $stmt->execute(['id' => $viewerId]);
        $username = $stmt->fetchColumn() ?: 'alice';
    }
    ...
    $error = 'Không kết nối được CSDL. Kiểm tra config/db.php, máy chủ MySQL (26.151.17.5) và mạng VPN/LAN của nhóm.';
}
?>
<!doctype html>
<html lang="vi">
                    ...
                    <?php elseif ($profile): ?>
                        ...
                        <div class="profile-switcher mt-4 text-center">
                            ...
                            <div class="d-flex flex-wrap gap-2 justify-content-center">
                                ...
                                ?>
                                <a href="profile.php?user=<?= e($u) ?>&viewer=<?= e($viewerId) ?>"
